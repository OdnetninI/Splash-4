...
void jacobcalc2(double ****x, double ****y, double ****z, long psiindex, long pid, long firstrow, long lastrow, long firstcol, long lastcol) {
  ...
  double** x_local = (double **) x[pid][psiindex];
  double** y_local = (double **) y[pid][psiindex];
  double** z_local = (double **) z[pid][psiindex];
  ...
  if (up_left_neighbor != -1) {
    x_local[0][0] = x[up_left_neighbor][psiindex][im-2][jm-2];
    y_local[0][0] = y[up_left_neighbor][psiindex][im-2][jm-2];
  }
  if (up_right_neighbor != -1) {
    x_local[0][jm-1] = x[up_right_neighbor][psiindex][im-2][1];
    y_local[0][jm-1] = y[up_right_neighbor][psiindex][im-2][1];
  }
  if (down_left_neighbor != -1) {
    x_local[im-1][0] = x[down_left_neighbor][psiindex][1][jm-2];
    y_local[im-1][0] = y[down_left_neighbor][psiindex][1][jm-2];
  }
  if (down_right_neighbor != -1) {
    x_local[im-1][jm-1] = x[down_right_neighbor][psiindex][1][1];
    y_local[im-1][jm-1] = y[down_right_neighbor][psiindex][1][1];
  }
  
  if (up_neighbor == -1) {
    if (left_neighbor != -1) {
      x_local[0][0] = x[left_neighbor][psiindex][0][jm-2];
      y_local[0][0] = y[left_neighbor][psiindex][0][jm-2];
    } else if (down_neighbor != -1) {
      x_local[im-1][0] = x[down_neighbor][psiindex][1][0];
      y_local[im-1][0] = y[down_neighbor][psiindex][1][0];
    }
    if (right_neighbor != -1) {
      x_local[0][jm-1] = x[right_neighbor][psiindex][0][1];
      y_local[0][jm-1] = y[right_neighbor][psiindex][0][1];
    } else if (down_neighbor != -1) {
      x_local[im-1][jm-1] = x[down_neighbor][psiindex][1][jm-1];
      y_local[im-1][jm-1] = y[down_neighbor][psiindex][1][jm-1];
    }
  } else if (down_neighbor == -1) {
    if (left_neighbor != -1) {
      x_local[im-1][0] = x[left_neighbor][psiindex][im-1][jm-2];
      y_local[im-1][0] = y[left_neighbor][psiindex][im-1][jm-2];
    } else if (up_neighbor != -1) {
      x_local[0][0] = x[up_neighbor][psiindex][im-2][0];
      y_local[0][0] = y[up_neighbor][psiindex][im-2][0];
    }
    if (right_neighbor != -1) {
      x_local[im-1][jm-1] = x[right_neighbor][psiindex][im-1][1];
      y_local[im-1][jm-1] = y[right_neighbor][psiindex][im-1][1];
    } else if (up_neighbor != -1) {
      x_local[0][jm-1] = x[up_neighbor][psiindex][im-2][jm-1];
      y_local[0][jm-1] = y[up_neighbor][psiindex][im-2][jm-1];
    }
  } else if (left_neighbor == -1) {
    if (left_neighbor != -1) {
      x_local[0][0] = x[left_neighbor][psiindex][im-2][0];
      y_local[0][0] = y[left_neighbor][psiindex][im-2][0];
    }
    if (down_neighbor != -1) {
      x_local[im-1][0] = x[down_neighbor][psiindex][1][0];
      y_local[im-1][0] = y[down_neighbor][psiindex][1][0];
  ...
  if (up_neighbor != -1) {
    ...
    double* y_neighbor = (double *) y[up_neighbor][psiindex][im-2];
    for (long col = 1; col <= lastcol; col++) {
  ...
  if (down_neighbor != -1) {
    ...
    double* y_neighbor = (double *) y[down_neighbor][psiindex][1];
    for (long col = 1; col <= lastcol; col++) {
  ...
  if (left_neighbor != -1) {
    ...
    double** y_neighbor = (double **) y[left_neighbor][psiindex];
    for (long row = 1; row <= lastrow; row++) {
  ...
  if (right_neighbor != -1) {
    ...
    double** y_neighbor = (double **) y[right_neighbor][psiindex];
    for (long row = 1; row <= lastrow; row++) {
