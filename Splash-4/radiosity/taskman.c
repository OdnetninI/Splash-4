...
*/
	long expected = LOAD(global->pbar_count);
	long result;
	do {
	  if( expected >= n_processors ) result = 1;
	  else result = expected + 1;
	} while(!CAS(global->pbar_count, expected, result));
	long bar_done = !(result < n_processors); 
...
/*
        ...
        UNLOCK(global->pbar_lock);
*/
        bar_done = !(LOAD(global->pbar_count) < n_processors);
        }
...
long _process_task_wait_loop()
{
    ...
    for( i = 0 ; i < 1000 && ! finished ; i++ )
        ...
        /*
			...
			UNLOCK(global->pbar_lock);
		*/
			if (LOAD(global->pbar_count) >= n_processors)
				finished = 1;
        }
        }
   ...
   */
{
    ...
    while( visit_count < max_visit )
    {
		...
		*/
		...
		int exit;
		long expectedNTasks = LOAD(tq->n_tasks);
		Task *expectedTop = LOAD(tq->top);
		do {
			exit = 0;
			if (expectedNTasks <= 0) break;
			exit = 1;

			do {
				task_found = 0;
				if (!expectedTop) break;
				task_found = 1;
			} while (!CAS(tq->top, expectedTop, expectedTop));
		} while (!CAS(tq->n_tasks, expectedNTasks, expectedNTasks));
		if (exit) break;
...
*/
	long expected = LOAD(global->task_counter);
	long result;
	do {
		flag = 0;
		if (expected == 0)
			flag = 1;
		if (expected+1 >= n_processors)
			result = 0;
		else
			result = expected+1;
	} while (!CAS(global->task_counter, expected, result));
