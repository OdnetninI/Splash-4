...
*/
    ...
    if (comp_last > NMOL1) {
	  ...
	  */
	  for ( dir = XDIR; dir  <= ZDIR; dir++) {
	    FETCH_ADD_DOUBLE(&(VAR[mol].F[DEST][dir][H1]), PFORCES[ProcID][mol][dir][H1]);
	    FETCH_ADD_DOUBLE(&(VAR[mol].F[DEST][dir][O]), PFORCES[ProcID][mol][dir][O]);
	    FETCH_ADD_DOUBLE(&(VAR[mol].F[DEST][dir][H2]), PFORCES[ProcID][mol][dir][H2]);
	  }
        }
        comp = comp_last % NMOL;
